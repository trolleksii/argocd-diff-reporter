...
func CompareInputFiles(from ytbx.InputFile, to ytbx.InputFile, out chan Diff) {
	cmpr := compare{
		settings: compareSettings{
			DetectRenames:                            true,
	...
	cmpr.documentNodes(from, to, out)
	close(out)
}
...
func (compare *compare) documentNodes(from, to ytbx.InputFile, out chan Diff) {
	type doc struct {
		node *yamlv3.Node
		idx  int
	}
...
func (compare *compare) basicType(node *yamlv3.Node) any {
	switch node.Kind {
	case yamlv3.DocumentNode:
